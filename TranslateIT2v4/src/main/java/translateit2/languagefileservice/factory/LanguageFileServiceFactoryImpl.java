...
public class LanguageFileServiceFactoryImpl implements LanguageFileServiceFactory {
	...
	private final Map<LanguageFileFormat, LanguageFileStorage> lngStorageServiceCache = 
			new HashMap<>();
	...
	public Optional<LanguageFileStorage> getService(LanguageFileFormat type) {
		Map<LanguageFileFormat, LanguageFileStorage>  services =
	...
	public List<LanguageFileFormat> listFormatsSupported() {	    	
		List<LanguageFileFormat> formats = 
				lngStorageServiceCache.entrySet().stream()
				.map(x -> x.getKey())
